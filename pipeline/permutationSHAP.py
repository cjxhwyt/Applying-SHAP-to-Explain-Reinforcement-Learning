# Step3: apply SHAP (Permutation Estimator) to explain how the bidder behaves on each data instance
import sys
...
if __name__ == '__main__':
    ...
    explainer = shap.explainers.Permutation(simulate_bids_permutation, X_train_obs, seed=config['random_seed'])

    start_time = time.time()
    shap_values_permutation = explainer(X_test_obs)
    print(f'Permutation explainer time: {timedelta(seconds=time.time() - start_time)}')

    with open('../shap/permutation_' + filename + '.pkl', 'wb') as file:
        pickle.dump(shap_values_permutation, file)
        print(f'Object successfully saved to "permutation_{filename}.pkl"')

    np.savetxt('../shap/permutation_' + filename + '.csv', shap_values_permutation.values, delimiter=",")
    shap.summary_plot(shap_values_permutation, X_test_obs, show=False)
    plt.savefig('../shap/permutation_' + filename + '.pdf')
    print('DONE SUMMARY')
