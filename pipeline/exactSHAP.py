# Step3: apply SHAP (Exact Estimator) to explain how the bidder behaves on each data instance
import sys
...
def simulate_bids_exp(all_contexts):
    result = []
...
if __name__ == '__main__':
    ...
    explainer = shap.explainers.Exact(simulate_bids_exp, X_train_obs)

    start_time = time.time()
    shap_values_exact = explainer(X_test_obs)
    print(f'Exact explainer time: {timedelta(seconds=time.time() - start_time)}')

    with open('../shap/exact_' + filename + '.pkl', 'wb') as file:
        pickle.dump(shap_values_exact, file)
        print(f'Object successfully saved to "exact_{filename}.pkl"')

    np.savetxt('../shap/exact_' + filename + '.csv', shap_values_exact.values, delimiter=",")
    shap.summary_plot(shap_values_exact, X_test_obs, show=False)
    plt.savefig('../shap/exact_' + filename + '.pdf')
    print('DONE SUMMARY')
