# Step3: apply SHAP (Sampling Estimator) to explain how the bidder behaves on each data instance
import sys
...
def simulate_a_bid(contexts):
    ...
    # sampling a true context vector
...
def simulate_bids_sampling(all_contexts):
    result = []
...
if __name__ == '__main__':
    ...
    explainer = shap.explainers.Sampling(simulate_bids_sampling, X_train_obs)

    start_time = time.time()
    shap_values_sampling = explainer(X_test_obs)
    print(f'Sampling explainer time: {timedelta(seconds=time.time() - start_time)}')

    with open('../shap/sampling_' + filename + '.pkl', 'wb') as file:
        pickle.dump(shap_values_sampling, file)
        print(f'Object successfully saved to "sampling_{filename}.pkl"')

    np.savetxt('../shap/sampling_' + filename + '.csv', shap_values_sampling.values, delimiter=",")
    shap.summary_plot(shap_values_sampling, X_test_obs, show=False)
    plt.savefig('../shap/sampling_' + filename + '.pdf')
    print('DONE SUMMARY')
